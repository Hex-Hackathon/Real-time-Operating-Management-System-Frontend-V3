...
const AddProduct = ({ orderId }) => {
  ...
  // const handleEdit = (index, newAmount) => {
  //   const availableStock = stock[index].quantity;

  //   if (newAmount > availableStock) {
  //     alert(
  //       `The entered quantity exceeds the available stock for ${stock[index].productName}`,
  //     );
  //   } else {
  //     setEditableData((prevData) => {
  //       const newData = [...prevData];
  //       newData[index].quantity = newAmount;
  //       return newData;
  //     });
  //   }
  // };
  ...
  const confirmtheOrder = () => {
    ...
    dispatch(handleOrderProcessConfirm(data)).then((result)=>{
      if(result.payload) location.reload();
    });
